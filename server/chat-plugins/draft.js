	...
	Nom(pk, user, self) {
		...
		if (this.order.length === this.order.indexOf(this.turn) + 1) {
			if (this.teams[this.turn].draftpicks.length === this.maxMons) {
				this.room.add('|html|<div style="' + greencss + '">' + hashColors(user.name, true) + ' has drafted the pokemon : <strong>' + pk + '</strong></div>');
				...
				this.room.add('|html|<div style="' + greencss + '">' + hashColors(user.name, true) + ' has drafted the Pokemon : <strong>' + pk + '.</strong><br>It is now <strong>' + this.turn + '</strong>\'s turn.</div>');
